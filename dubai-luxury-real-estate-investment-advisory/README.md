# Dubai Luxury Real Estate Investment Advisory

A research led investment advisory case study comparing **Golf Acres and Greenspoint in Emaar South, Dubai South** for a residential end user mandate with a firm **AED 2,000,000 budget**.

The project combines market research, property comparison, affordability analysis, Dubai Land Department transaction evidence, risk assessment, weighted decision scoring, and executive communicatio[...]

## Project Preview

![Dubai Luxury Real Estate Investment Advisory](Dubai%20Luxury%20Real%20Estate.png)

## Project Deliverables

This project includes both the executive decision presentation and the detailed research supporting the analysis.

| Deliverable | Purpose |
| --- | --- |
| [Investment Advisory Presentation](./Dubai_Luxury_RE_Investment_Advisory.pptx) | Executive presentation translating the research, comparison, risk assessment, scoring framework, and recommendation into a decision focused format |
| [Investment Advisory Research Report](./Emaar_South_Investment_Advisory_Report.pdf) | Detailed research and evidence base supporting the market analysis, project comparison, transaction review, assumptions, limitations, and recommendation framework |

### Recommended Viewing Order

**1. Investment Advisory Presentation**

Start with the presentation for the concise executive version of the analysis and recommendation.

**2. Investment Advisory Research Report**

Review the research report for the detailed evidence, sourcing, methodology, assumptions, and limitations supporting the advisory.

## Business Question

The case study addresses a practical investment decision:

**Which of the two Emaar South projects provides the stronger fit for a residential end user with a AED 2,000,000 budget?**

The analysis compares Golf Acres and Greenspoint using a structured decision framework rather than relying on general market sentiment.

## Client Mandate

| Requirement | Scope |
| --- | --- |
| Budget | AED 2,000,000 |
| Purpose | Residential end user |
| Location | Emaar South, Dubai South |
| Projects | Golf Acres and Greenspoint |
| Decision Style | Evidence based and auditable |

## Analytical Framework

The comparison considers several decision dimensions:

* Affordability and budget fit
* Livability and connectivity
* Future appreciation exposure
* Developer quality
* Liquidity and risk profile
* Investment fundamentals

A weighted scoring approach was used to bring these factors into one structured comparison.

## Research Approach

The project was supported by detailed market research using publicly available sources.

The research considered information from sources including:

* Emaar Properties
* Dubai Land Department Open Data
* Property Finder
* Bayut
* Propsearch
* Dubai Aviation Engineering Projects
* Expo City Dubai research
* UAE Central Bank publications
* Emaar corporate financial disclosures

Where information could not be independently verified, it was treated as unavailable rather than estimated.

Forward looking market information was treated as third party projection rather than guaranteed performance.

## Key Decision Finding

Budget compatibility is the clearest differentiator in this client mandate.

Golf Acres included unit options within the stated AED 2,000,000 ceiling, while the Greenspoint entry pricing reviewed in the research exceeded the budget.

Because both developments sit within the same wider Emaar South environment and share the same broad developer context, affordability became a major decision factor in the final comparison.

## Transaction Evidence

The research also examined available Dubai Land Department transaction information.

Golf Acres showed recorded transaction activity within the research period, while comparable project level transaction evidence for Greenspoint was not located in the public sources reviewed at th[...]

This difference was treated as an analytical consideration rather than proof of future liquidity.

## Recommendation Framework

The final recommendation was produced through a weighted decision model designed for this specific client mandate.

The model supports a structured comparison, but the resulting score should be understood as a decision framework rather than an objective market rating.

The recommendation therefore reflects the client's budget, purpose, and decision criteria rather than claiming that one project is universally superior.

## Risk Perspective

The analysis considers investment risk alongside opportunity.

Areas reviewed include:

* Budget exposure
* Market liquidity
* Supply risk
* Infrastructure dependency
* Developer considerations
* Holding period
* Market and regulatory context

The purpose is to make assumptions and tradeoffs visible before reaching a recommendation.

## Executive Communication

The analytical work was translated into an executive investment advisory presentation.

The presentation condenses the research into a decision focused narrative covering the client mandate, market context, project comparison, risk assessment, scoring framework, and final recommenda[...]

This demonstrates the ability to move from detailed research to concise executive communication.

## Tools and Skills Demonstrated

| Area | Application |
| --- | --- |
| Market Research | Public source research and evidence gathering |
| Business Analysis | Client mandate and decision criteria definition |
| Commercial Analysis | Pricing, affordability and comparative assessment |
| Data Validation | Cross source checking and DLD transaction review |
| Risk Analysis | Structured investment risk assessment |
| Decision Modelling | Weighted comparison framework |
| Executive Reporting | Recommendation focused presentation |
| Generative AI | AI assisted research, synthesis and analytical workflow |

## What This Project Demonstrates

This project goes beyond dashboard development.

It demonstrates how I approach a business decision from beginning to end:

**define the mandate -> gather evidence -> validate the information -> compare alternatives -> assess risk -> structure the decision -> communicate the recommendation**

The emphasis is on analytical judgement, traceable evidence, and clear communication.

## Important Context

This is a portfolio case study demonstrating analytical and advisory capability.

It should not be interpreted as financial, legal, property brokerage, or regulated investment advice.

Pricing, availability, market conditions, infrastructure timelines, and other forward looking information can change and should be independently reconfirmed before any real transaction.

## About Me

I am Joshua Abejide, a UAE based Business Analyst and analytics professional with experience across commercial and operational environments.

My analytics toolkit includes Power BI, Excel, SQL, Tableau, Python, Power Query, DAX, and Generative AI tools.

[LinkedIn](https://www.linkedin.com/in/josh-abejide-891245107/) | [GitHub Profile](https://github.com/Josh-notes) | [Analytics Portfolio](https://github.com/Josh-notes/data-analysis-portfolio)
